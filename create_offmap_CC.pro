...
pro create_offmap, $
    ...
    ; ....................................................................................................................................
    ; Key    
    ; Names of two DEM tif files. tif1: master DEM, tif2: slave DEM
    ; tif1 should be prior to tif2 (ie. the date of tif1 should be before the date of tif2)
    tif1, tif2, $ 
    ;
    ; Short name for those two images (ex: '20110412'), id1 and id2
    id1, id2, $
    ; x and y coordinates for cropping, north-west corner in meters 
    pos_x, pos_y, $
    ...
    min_meltrate=min_meltrate, $ ; 
    max_meltrate=max_meltrate, $ ; 
    min_dhdt=min_dhdt, $ ; 
    max_dhdt=max_dhdt, $ ; 
    ; sclx : scale factor for offsets x (default 0) 0.1 equals 10% speed up
    ; scly : scale factor for offsets y (default 0) 0.1 equals 10% speed up 
    sclx=sclx, $ ; 
    scly=scly, $ ; 
    ...
    shape_is_grounded=shape_is_grounded, $ ; 
    ...
    shapefile_meter=shapefile_meter, $ ; 
...
if ~exist(tif1) or ~exist(tif2) then begin
    ...
    print,"sclx (num) : scale factor for offsets x (default 0) 0.1 = 10% speed up"
    print,"scly (num) : scale factor for offsets y (default 0) 0.1 = 10% speed up"
...
if (~exist(ampcor_loc)) then ampcor_loc = "/mnt/pennell0/arnaud/"
...
if (~exist(sclx)) then sclx = 0
if (~exist(scly)) then scly = 0
...
if (adapt && a_priori) then begin
	print,"Error, you have to chose between /comp and /apriori" ;which i s /comp and apriori??
        cd,'..'
        return
endif
...
foreach id, ids do begin
	...
	file_copy, "DEM_gc_par", id + ".DEM_gc_par" ;DEM_gc_par (getting formate of par)
	
        ; Getting the pixel scale
        tif_tmp = query_tiff(id + ".tif", geotiff = var)
	
	; correction by CC SEP 2018, 
	print, "var.modelpixelscaletag[0] " + string(var.modelpixelscaletag[0],format='(F16.3)')
	print, "var.modelpixelscaletag[1] " + string(var.modelpixelscaletag[1],format='(F16.3)')
	...
	; Modifying parameters for the next two-image calculations
	...
	; Creating DEM .dat image for two tif files
	...
	close,1
        ...
        plot_contour,id+".small.dat",paramfile=id+".small.dat.par",ct_num=15,n_levels=200,save_img=id+".small",title="Original WV DEM data for " + id
...
if ~skip_offset then begin
    ...
    ; Prepare ampcor_apriori from annual velocity map
...
compute_diff,id1,id2,shapefile_loc,scale_rg,scale_line,min_dhdt=min_dhdt,max_dhdt=max_dhdt,sclx=sclx,scly=scly,shape_is_grounded=shape_is_grounded,shapefile_meter=shapefile_meter,skip_offset=skip_offset,scale_skip_offset=rgspac,testing=testing,geotiff=geotiff,post_north=pos_y,post_east=pos_x,/verbose,apriori=a_priori

;X) Converting in basal melt rate
; add scale_skip_offset variable, modification by CC SEP 2018
if ~skip_offset then diff_to_meltrate,id1,id2,shapefile_loc,scale_rg,scale_line,min_meltrate=min_meltrate,max_meltrate=max_meltrate,filter=filter,sz_filter=sz_filter,antarctica=antarctica,shape_is_grounded=shape_is_grounded,shapefile_meter=shapefile_meter,apriori=a_priori,geotiff=geotiff,post_north=pos_y,post_east=pos_x,scale_skip_offset=rgspac
